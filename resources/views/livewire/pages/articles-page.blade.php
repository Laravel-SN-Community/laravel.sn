        ...
        <div class="max-w-7xl mx-auto px-8 sm:px-12 lg:px-16">
            <div class="max-w-4xl">
                ...
                <div class="inline-flex items-center gap-2 mb-8">
                    ...
                    <span class="text-sm font-medium text-red-600 uppercase tracking-wider">{{ __('Articles') }}</span>
                </div>
        ...
        <div class="max-w-7xl mx-auto px-8 sm:px-12 lg:px-16">
            @if($articles->count() > 0)
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($articles as $article)
                        <article class="bg-white rounded-xl border border-gray-200 overflow-hidden hover:border-red-300 transition-all duration-200 flex flex-col group">
                            <!-- Article Cover Image -->
                            ...
                            <div class="p-6 flex flex-col gap-4 flex-1">
                                ...
                                @if($article->tags->count() > 0)
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($article->tags as $tag)
                                            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-gray-50 text-gray-700 text-xs font-medium rounded-full border border-gray-200">
                                ...
                                <a wire:navigate href="{{ route('article.show', $article->slug) }}" class="block">
                                    <h4 class="text-xl font-bold text-gray-900 group-hover:text-red-600 transition-colors line-clamp-2">
                                        {{ $article->title }}
                                    </h4>
                                </a>
                                ...
                                <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                    <div class="flex flex-col gap-2">
                                        <div class="flex items-center text-gray-500 text-xs">
                                            ...
                                            <span class="text-gray-900 font-medium">{{ $article->published_at->format('d M Y') }}</span>
                                        </div>
                                        <div class="flex items-center text-gray-500 text-xs">
                                            <span class="text-gray-600 font-light">{{ number_format($article->views_count) }} {{ __('views') }}</span>
                                        </div>
                                    </div>
                                    <a wire:navigate href="{{ route('article.show', $article->slug) }}"
                                       class="inline-flex items-center gap-1 text-red-600 text-sm font-semibold hover:text-red-700 transition-colors group/link">
                                        {{ __('Read more') }}
            ...
            @else
                ...
                <div class="text-center py-16 max-w-2xl mx-auto">
                    ...
                    <h3 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4 leading-tight tracking-tight">{{ __('No article found') }}</h3>
