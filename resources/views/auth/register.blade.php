            ...
            <div
                ...
                <div class="flex flex-col justify-between px-8 py-12 sm:px-12">
                    <div class="space-y-8">
                        ...
                        <div class="space-y-3">
                            <h1 class="text-3xl font-semibold text-slate-900 sm:text-4xl">{{ __('Create your account') }}</h1>
                            <p class="text-base text-slate-500">
                                {{ __('Join the Laravel Senegal community, discover exclusive events, and collaborate with fellow artisans.') }}
                            </p>
                        </div>
                        ...
                        <form method="POST" action="{{ route('register') }}" class="space-y-6">
                            ...
                            <div class="space-y-2">
                                <x-label for="name" value="{{ __('Full name') }}" class="text-sm font-medium text-slate-700" />
                                <x-input
                                    id="name"
                            ...
                            <div class="grid gap-4 sm:grid-cols-2">
                                ...
                                <div class="space-y-2">
                                    <x-label for="password_confirmation" value="{{ __('Confirm password') }}" class="text-sm font-medium text-slate-700" />
                                    <x-input
                            ...
                            @if (Laravel\Jetstream\Jetstream::hasTermsAndPrivacyPolicyFeature())
                                <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                                    <x-label for="terms" class="space-y-3">
                                        <div class="flex items-start gap-3">
                                            <x-checkbox name="terms" id="terms" required class="mt-1" />
                                            <span>
                                                {!! __('I agree to the :terms_of_service and :privacy_policy', [
                                                        'terms_of_service' => '<a target="_blank" href="'.route('terms.show').'" class="font-semibold text-indigo-500 transition hover:text-indigo-600">'.__('Terms of Service').'</a>',
                                                        'privacy_policy' => '<a target="_blank" href="'.route('policy.show').'" class="font-semibold text-indigo-500 transition hover:text-indigo-600">'.__('Privacy Policy').'</a>',
                                                ]) !!}
                                            </span>
                                        </div>
                                    </x-label>
                    ...
                    <div class="mt-8 space-y-6">
                        <div class="space-y-2">
                            <p class="text-sm font-semibold uppercase tracking-wide text-slate-400">{{ __('Or sign up with') }}</p>
                        ...
                        <p class="text-sm text-slate-500">
                            ...
                            <a href="{{ route('login') }}" class="font-semibold text-indigo-500 transition hover:text-indigo-600">
                                {{ __('Sign in') }}
                            </a>
                        </p>
                    </div>
                    ...
                    <div class="relative z-10 flex h-full flex-col justify-between">
                        <div class="space-y-4">
                            ...
                            <h2 class="text-3xl font-semibold leading-tight text-white sm:text-4xl">
                                {{ __('Collaborez, apprenez et faites grandir vos projets.') }}
                            </h2>
                            <p class="text-base text-white/80">
                                {{ __('Participez à des événements exclusifs, accédez à des ressources partagées et créez des connexions durables avec les membres du réseau Laravel Sénégal.') }}
                            </p>
                        </div>
